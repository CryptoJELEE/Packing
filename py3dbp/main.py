...
class Bin:
    ...
    def __init__(self, partno, WHD, max_weight,corner=0,put_type=1,is_pallet=False):
        ''' '''
        self.partno = partno
        self.width = WHD[0]
        self.height = WHD[1]
        ...
        self.is_pallet = is_pallet  # 파레트 여부
        ...
        for i in range(0, len(rotate)):
            ...
            if fit:
                ...
                if self.fix_point == True :
                    ...
                    # 파레트 모드: 바닥에 직접 올라가는 경우 지지면 비율 체크 스킵
                    if self.check_stable == True :
                        # 파레트 모드이고 바닥에 직접 올라가는지 확인
                        is_on_pallet_floor = (z < 1.0)  # 거의 바닥에 닿음
                        is_on_other_items = False
                        
                        # 다른 아이템 위에 올라가는지 확인
                        for i in self.fit_items:
                            if abs(z - i[5]) < 1.0 and i[5] > 1.0:  # 같은 높이에 다른 아이템이 있고 바닥이 아님
                                is_on_other_items = True
                                break
                        
                        # 파레트이고 바닥에 직접 올라가는 경우는 안정성 검사 스킵
                        if self.is_pallet and is_on_pallet_floor and not is_on_other_items:
                            # 파레트 바닥에 직접 올라가는 경우 안정성 검사 통과
                            pass
                        else:
                            # 기존 안정성 검사 로직 (다른 아이템 위에 올라가는 경우)
                            ...
                            for i in self.fit_items:
                                ...
                                if abs(z - i[5]) < 1.0:  # 같은 높이에 있는 아이템
                            ...
                            # 다른 아이템 위에 올라가는 경우만 지지면 비율 체크
                            if is_on_other_items and support_area_upper / item_area_lower < self.support_surface_ratio :
